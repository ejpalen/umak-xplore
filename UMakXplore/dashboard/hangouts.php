<div class="summary-container">
  <div class="summary">
    <img src="../media/icons/highestcrowdIcon.png" alt="">
    <h1>9432</h1>
    <p>Highest Crowd</p>
  </div>
  <div class="summary">
    <img src="../media/icons/crowdIcon.png" alt="">
    <h1>1.1k</h1>
    <p>Average Crowd</p>
  </div>
  <div class="summary">
  ...
  <div class="summary">
    <img src="../media/icons/openHangoutsIcon.png" alt="">
    <h1>143</h1>
    <p>Open Hangouts</p>
  </div>
</div>
...
<div class="users-filter-wrapper">
  <input type="text" placeholder="Find Hangouts">
...
<div class="add-option">
  <a href="../dashboard/?hangoutsOption=add">
  <span>&#43;</span>
  <p>Add Hangouts</p>
  </a>
</div>
...
while ($row = mysqli_fetch_assoc($resultBldg)) {
  ...
  while ($floor_row = mysqli_fetch_assoc($floor_names)) {
    ...
    $hangouts_query = mysqli_query($conn, "SELECT * FROM HANGOUTS WHERE floor_id = '{$floor_row['id']}'");
    
    if (mysqli_num_rows($hangouts_query) > 0) {
      echo '<div class="users-table-wrapper">'; 
      echo '<br><h3>' . $floor_row['name'] . '</h3><br>
      <table>
      <tr id="user-table-header">
        <th>Hangouts ID</th>
        <th>Hangouts Filter</th>
        <th>Hangouts Name</th>
        <th>Hangouts Image</th>
        <th>Hangouts Description</th>
        <th>Hangouts Live Crowd Tracker</th>
        <th>Hangouts Status</th>
        <th>Hangouts Operating Hours</th>
        <th>Hangouts Saved</th>
        <th id="action-header">Action</th>
      </tr>';

      while ($hangouts_row = mysqli_fetch_assoc($hangouts_query)) {
        echo '<tr>
        <td>' . $hangouts_row['id'] . '</td>
        <td>' . $hangouts_row['filter'] . '</td>
        <td>' . $hangouts_row['name'] . '</td>
        <td> <img src="';
        

        if (file_exists($hangouts_row['image_url'])) {
          echo $hangouts_row['image_url'] ;
        } else {
          echo '../media/UmakDiscovery/discoveriesHangoutsIcon.png';
        }
        
        
        echo '"></td>
        <td>' . $hangouts_row['description'] . '</td>
        <td>' . $hangouts_row['tracker'] . '</td>
        <td>' . $hangouts_row['status'] . '</td>
        <td>' . $hangouts_row['time'] . '</td>
        <td>' . $hangouts_row['saved'] . '</td>
        <td>
        <div class="action-container">
          <a href="../dashboard/?hangoutsOption=edit" class="user-action-edit">
          ...
          <a href="../dashboard/?hangoutsOption=delete" class="user-action-delete">
